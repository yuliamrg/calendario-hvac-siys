...
function parseCities(table, warnings) {
  ...
  for (const { row, rowNumber } of sourceRows) {
    ...
    if (!isPresentCell(rowValue(row, indexes.id))) {
      warnings.push(makeWarning(
        "city_without_id",
...
function resolveRequiresAppColumn(table, warnings) {
  ...
  const columnK = 10;
  const headerIsBlank = !isPresentCell(table.headers[columnK]);
...
function parseSites(table, clients, warnings) {
  ...
  for (const { row, rowNumber } of sourceRows) {
    ...
    if (indexes.requiresApp >= 0 && isPresentCell(appRaw) && requiresApp === null) {
      warnings.push(makeWarning(
...
function parseEquipmentHints(table, sites, warnings) {
  ...
  for (const { row, rowNumber } of sourceRows) {
    ...
    for (const month of MONTHS) {
      if (isPresentCell(rowValue(row, indexes.months[month]))) aggregate.scheduledMonths.add(month);
    }
  }
...
function sanitizeFileMetadata(metadata, warnings) {
  ...
  } else if (isPresentCell(metadata.sha256)) {
    warnings.push(makeWarning(
      "invalid_source_hash",
